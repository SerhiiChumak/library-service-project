...
from chat_bot.telegram_bot import application
from chat_bot.views import send_message_to_chat, send_private_message

GETPOST_URL = reverse_lazy("chat_bot:telegram_bot")
TELEGRAM_LINK_URL = reverse("chat_bot:telegram_links")
...
class TelegramWebhookTest(TestCase):
    ...
    @patch("chat_bot.views.application.process_update")
    @patch("chat_bot.views.application.initialize")
...
class TestSendMessages(TestCase):
    ...
    @patch("chat_bot.views.requests.post")
    def test_send_message_to_public_chat(self, mock_post):
        """test function actually send message to public chat"""
    ...
    @patch("chat_bot.views.requests.post")
    def test_send_message_to_private_chat(self, mock_post):
        """test function actually send message to private chat"""
        mock_response = MagicMock()
        mock_response.status_code = 200
        mock_post.return_value = mock_response
